...
```{r}
isaria <- isaria %>% group_by(satz_id, variante_H2O, wiederholung, datum_messung) %>%
    summarise(across(ibi:irmi, ~mean(., na.rm=TRUE))) %>%
...
<!-- #Tageslichtsunden summieren ab Auflaufen (statt Aussaat) -->
<!-- ```{r} -->
<!-- db1 <- dbConnect(RSQLite::SQLite(), db) -->
<!-- das <- dbGetQuery(db1, "SELECT  -->
<!-- satz_id, datum_aussaat, datum_ernte, datum_auflaufen_BBCH_09   -->
<!--                   FROM Spinat_Saetze") -->

<!-- das <- das %>% mutate_at(c("datum_aussaat","datum_ernte", "datum_auflaufen_BBCH_09"),  -->
<!--                          ~as_date(.)) %>% -->
<!--   mutate(das = datum_ernte - datum_aussaat, -->
<!--          dae = datum_ernte - datum_auflaufen_BBCH_09)  -->
...
<!-- ```{r} -->
<!-- wetter2 <- wetter %>%    -->
<!--   left_join(das %>% select(satz_id, datum_aussaat, datum_auflaufen_BBCH_09),  -->
<!--             by = "satz_id") %>% -->
<!--   mutate(dae = (datum_wetter - datum_auflaufen_BBCH_09) %>% as.numeric, -->
<!--          das = (datum_wetter - datum_aussaat) %>% as.numeric) %>% -->

<!--   mutate(cumsum_direction = ifelse(dae < 0,"negative", "positive"))%>% -->
<!--   group_by(satz_id, cumsum_direction) %>% -->
<!--   arrange(abs(dae), .by_group = TRUE) %>% -->
<!--   mutate(Tmean_th9_25_gradC_sum_dae = cumsum(Tmean_th9_25_gradC), -->
<!--          tageslicht_h_th9_sum_dae = cumsum(tageslicht_h_th9), -->
<!--          tageslicht_h_sum_dae = cumsum(tageslicht_h)) %>%  -->

<!--   arrange(dae, .by_group = TRUE) %>% #sortierung wieder rückgängig machen -->


<!--   mutate_at(c("Tmean_th9_25_gradC_sum_dae", "tageslicht_h_th9_sum_dae"),  -->
<!--             ~ifelse(dae<0, -., .)) %>% -->
<!--   mutate(correction_temp = abs(max(Tmean_th9_25_gradC_sum_dae)), -->
<!--          correction_licht = abs(max(tageslicht_h_th9_sum_dae)), -->
<!--          correction_licht2 = abs(max(tageslicht_h_sum_dae))) %>% -->

<!--   group_by(satz_id) %>% -->
<!--   mutate_at(c("Tmean_th9_25_gradC_sum_dae"), ~ifelse(dae<0, .+correction_temp, .)) %>% -->
<!--   mutate_at(c("tageslicht_h_th9_sum_dae"), ~ifelse(dae<0, .+correction_licht, .)) %>% -->
<!--   mutate_at(c("tageslicht_h_sum_dae"), ~ifelse(dae<0, .+correction_licht2, .)) %>% -->

<!--   select(-cumsum_direction, -correction_temp, -correction_licht, -correction_licht2) -->
...
input_tabelle <- input_tabelle %>% 
    ...
    select(-niederschlag_mm, -bewaesserung_mm, -wasser_input,-wiederholung, -satz_id, -variante_H2O, -zeit_messung) %>%
    tidyr::drop_na()
```
