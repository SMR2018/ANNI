...
p1 + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
p2 + theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
```
NeuralNetTools::olden(nn) zeigt auch Vorzeichen: Tmin, Tage_seit_Aussaat und windgeschwindigkeit sind kontraproduktiv für das Neuronale Netzwerk. Sie wurden daher in einem weiteren Schritt entfernt.
...
```{r}
p1 <- ggplot(df_all, aes(actual_value, predicted_value,
