...
fwrite(scaling_data, file = "../data/derived_data/Model2/scaling_data.csv")
```
...
#Für einen Satz von oben in chronologischer Reihenfolge erstellen:
```{r}
idx_einSatz <- 949:981
satz_raw <- data[idx_einSatz,]

#X definieren: #Ab Spalte 7
idx_x <- 7:ncol(data)
#Target definieren: #erste 6 Spalten sind Y-Werte (nFK 0 - 60 cm)
idx_y <- 1:6


##Daten zusammenstellen:
satz_x <- satz_raw[,idx_x]
#Daten skalieren. 
satz_x <- scale(satz_x, center = m, scale = s) #m und s oben bei Modellerstellung

satz_target <- satz_raw[,idx_y]
```

```{r}
new_model %>% evaluate(satz_x, satz_target)
pred <- new_model %>% predict(satz_x)
mean((satz_target-pred)^2) 

```

```{r}
x2 <- satz_target %>% as.data.frame() %>% 
    mutate(Tage_seit_Aussaat = data[idx_einSatz,13]) %>% #Spalte 13 ist DAS
    tidyr::pivot_longer(cols = -Tage_seit_Aussaat, values_to = "measured", names_to = "depth")

y2 <- pred %>% as.data.frame() %>% 
    tidyr::pivot_longer(cols = everything(), values_to = "predicted") %>% select(predicted)
...
ggplot(result) +
    ...
    geom_line(aes(Tage_seit_Aussaat, measured), col = "black") + 
    geom_line(aes(Tage_seit_Aussaat, predicted), col = "red") + 
    ylab("nFK (%)") + 
    facet_grid(depth~.) + 
    theme_bw() + 
...
# ANNI Für jeden Tag einzeln hintereinander im Loop ausführen:
```{r}
pred2 <- matrix(ncol = 6, nrow = nrow(satz_x)+1)
satz_target_init <- matrix(ncol = 6, nrow = nrow(satz_x)+1) #Leere nfk und ersten Tag füllen. Bei Praxis dann mit 100%
satz_target_init[1,] <- satz_target[1,]
    
for (i in 1:nrow(satz_x)){
new_model %>% evaluate(matrix(nrow = 1, satz_x[i,]), matrix(nrow = 1, satz_target_init[i,]))
pred2[i,] <- new_model %>% predict(matrix(nrow = 1, satz_x[i,]))
satz_target_init[i+1,] <-  pred2[i,]
}

satz_target_init <- satz_target_init[-1,]
```
