...
importance_100 <- map(1:20, ~olden_per_rep(i = .x, nn_list = nn_list)) %>% bind_cols()
```
```{r}
tt <- t(importance_100) %>% as_data_frame() 
...
p2.3 <- p2.2 + 
    geom_errorbar(data = quantiles_olden, aes(x = parameter, 
                                              ymin = lower, 
...
#                                 input = "test_Daten"))%>% bind_cols(idx_saetze[-idx,])
# df_all <- bind_rows(df1, df2)
```

```{r}
p1 <- ggplot(df1, aes(actual_value, predicted_value,
...
#Modellguete R2:
summary(lm(predicted_value ~actual_value, data = df1))$adj.r.squared
#Modellguete MSE:
MSE.nn
```

```{r}
# ggsave(plot = p1, "../graphics/ANNI_actual_predicted.png", device = "png", 
#        width = 4, height = 4, dpi = 300)
```

#Zeireihe plotten:
```{r}
df1$tage_seit_aussaat <- (input_df %>% pull(tage_seit_aussaat))
df1$wiederholung <- (input_df %>% pull(wiederholung))
...
```{r}
ggplot(df1 %>% filter(satz_id == 4, variante_H2O == "Wfull_plus", wiederholung == "a"), 
