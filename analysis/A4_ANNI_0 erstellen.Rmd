...
```{r}
#plot(nn)
# tree_plots <- map(list(nn0020_best, nn2040_best, nn4060_best),
#     ~p1 <- 
plot(nn0020_best[["nn"]],col.hidden = 'darkgreen', 
...
all_predicted <- map_df(list(S0020_cm = nn0020_best, S2040_cm = nn2040_best, S4060_cm = nn4060_best),
                        ~.x[["df1"]], .id = "BodenTiefe")
plot_measured_predicted(data = all_predicted) + aes(col=BodenTiefe, group = BodenTiefe) +
...
map(list(S0020_cm = nn0020_best, S2040_cm = nn2040_best, S4060_cm = nn4060_best),
    ~add_DAS_to_output_df(data = .x[["df1"]]))
