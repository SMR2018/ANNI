...
input_df.scaled <- as.data.frame(scale(input_df))
...
#ANNI
```{r}
nn=neuralnet(T0060_mm~Tage_seit_Aussaat+T0020_nFK_Vortag+T2040_nFK_Vortag+ T4060_nFK_Vortag+
                 Tmean+RF+Glob+RHmin+V2+wasser_input_Vortag,
             data=input_df.scaled[idx,],
             hidden=c(5,3),
             act.fct = "logistic",
             linear.output = FALSE,
...
```{r}
Predict=compute(nn,input_df.scaled[-idx,])
#Predict$net.result
```
...
```{r}
ggplot(data.frame(actual_value =input_df.scaled[-idx,1],
                  predicted = Predict$net.result),
       aes(actual_value, predicted)) + 
    geom_point() + 
...
Predict_ <- Predict$net.result*(max(input_df$T0060_mm)-
                                    min(input_df$T0060_mm))+min(input_df$T0060_mm)

test.r <- (input_df.scaled[-idx,])*(max(input_df$T0060_mm)-min(input_df$T0060_mm))+min(input_df$T0060_mm)

MSE.nn <- sum((test.r - Predict_)^2, na.rm = TRUE)/nrow(input_df.scaled[-idx,])
MSE.nn
```
