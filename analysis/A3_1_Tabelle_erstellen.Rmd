...
input_tabelle <- 
    ...
    select(-niederschlag_mm, -bewaesserung_mm, -wasser_input,
           ...
           #-wiederholung, -variante_H2O, -zeit_messung
    ) %>%
...
#gleiche Intervalle für LSTM-Anni
```{r}
i1 <- input_tabelle %>% 
    group_by(satz_id, variante_H2O, wiederholung) %>%
...
i14_sliced <- i14 %>%
    mutate(nx = 1:n(), .after = "variante_H2O")

i14plus_sliced <- i14plus %>% slice(idx1,(n()-idx2):n()) %>%
    mutate(nx = 1:n(), .after = "variante_H2O")

i30plus_sliced <- i30plus %>% slice(idx1,(n()-idx2):n(), idx3) %>%
    mutate(nx = 1:n(), .after = "variante_H2O")

df_same_lengths <- bind_rows(i14_sliced, i14plus_sliced, i30plus_sliced) %>%
    group_by(satz_id, variante_H2O, wiederholung) %>% 
    mutate(sub1  = case_when(nx<=14 ~ 1,
                             between(nx,15,28) ~ 2,
                             nx > 28 ~ 3,
                             T ~ 999)) %>% 
    # group_by(satz_id, variante_H2O, wiederholung, sub1) %>%
    # summarise(nn2 = n()) %>%
    ungroup()


input_tabelle <- df_same_lengths %>% 
    select(-satz_id, -wiederholung, -variante_H2O, -nx, -nn, -sub1,
...
#             file = "../data/derived_data/input_tabelle_2020_2022_20230905_20cm_LSTM_complete.csv",
