...
<!-- ## "Stations_id" der Stationen mit Globalstrahlung für "recent" und "historical" auslesen. -->
<!-- ```{r} -->
<!--  id_globalstrahlung <-    meta1 %>% -->
<!-- select(Stations_id, res, var, per) %>% -->
<!-- filter(var == "solar" & res == "10_minutes") %>% -->
<!-- group_by(Stations_id, per) %>% -->
<!-- summarise(n =n(), .groups = "drop") %>% -->
<!-- pivot_wider(id_cols = Stations_id, names_from = per, values_from = n) %>% -->
<!-- filter(if_any(c("recent", "historical"), ~!is.na(.))) %>% -->
...
<!-- ## "Stations_id" der Stationen mit Klimadaten auslesen (Subset von Stationen mit Globalstrahlung) -->
<!-- ```{r} -->
<!-- id_klima <- meta1 %>%  -->
<!-- select(Stations_id, res, var, per) %>% -->
<!-- filter(var == "kl" & res == "daily") %>%  -->
<!-- group_by(Stations_id, per) %>%  -->
<!-- summarise(n =n(), .groups = "drop") %>% -->
<!-- pivot_wider(id_cols = Stations_id, names_from = per, values_from = n) %>%  -->
<!-- filter(if_any(c("recent", "historical"), ~!is.na(.))) %>%  -->
...
<!-- #Subset beider Stations-Indices -->
<!-- idx_select <- match(id_globalstrahlung, id_klima) -->
<!-- id_complete <- id_klima[idx_select] -->
<!-- ``` -->

<!-- ## "Stations_id" der Stationen mit rel.Luftfeuchte auslesen (Subset von Stationen mit Globalstrahlung) -->
<!-- ```{r} -->
<!-- id_moisture <-  -->
<!-- meta1 %>%  -->
<!-- select(Stations_id, res, var, per) %>% -->
<!-- filter(var == "moisture" & res == "subdaily") %>% #fuer min und max Berechnung -->
<!-- group_by(Stations_id, per) %>%  -->
<!-- summarise(n =n(), .groups = "drop") %>% -->
<!-- pivot_wider(id_cols = Stations_id, names_from = per, values_from = n) %>%  -->
<!-- filter(if_any(c("recent", "historical"), ~!is.na(.))) %>%  -->
...
<!-- #Subset beider Stations-Indices -->
<!-- idx_select <- as.numeric(na.omit(match(id_complete, id_moisture))) -->
<!-- id_complete <- id_moisture[idx_select] -->
<!-- ``` -->
...
<!-- meta1 %>%  -->
<!-- select(Stations_id, res, var, per) %>% -->
<!-- filter(var == "wind" & res == "subdaily") %>% #in "kl" ist alles NA -->
<!-- group_by(Stations_id, per) %>%  -->
<!-- summarise(n =n(), .groups = "drop") %>% -->
<!-- pivot_wider(id_cols = Stations_id, names_from = per, values_from = n) %>%  -->
<!-- filter(if_any(c("recent", "historical"), ~!is.na(.))) %>%  -->
...
<!-- #climate/subdaily/wind/recent/ -->
<!-- #climate/subdaily/wind/historical/ -->

<!-- #Subset beider Stations-Indices -->
<!-- idx_select <- as.numeric(na.omit(match(id_complete, id_wind))) -->
<!-- id_complete <- id_wind[idx_select] -->
<!-- ``` -->


<!-- ## "Stations_id" der nFK-Daten auslesen: -->
<!-- ```{r} -->
<!-- idx_select <- as.numeric(na.omit(match(id_complete, id_nFK))) -->
<!-- id_complete <- id_nFK[idx_select] -->
<!-- ``` -->
289 Stationen liefern alle Parameter zu heutigen und historischen Werten.
...
here::i_am(path = "analysis/A7_Stationsdaten_DE_input_ANNI.Rmd")
...
# dir = here::here("data/raw_data/DWDdata"), 
# quiet=TRUE, force=NA)
...
# In eine lange Tabelle umformen:
clim_df <- bind_rows(clim)
# Speichern der Wetterdaten als RDS
saveRDS(object = clim_df, 
file = "../data/derived_data/DWD_data/kl_Klima/kl_tagesmittel_290Stationen.rds")
```

```{r}
#gespeicherte Klima-Daten einlesen
clim_df <- readRDS(file = "../data/derived_data/DWD_data/kl_Klima/kl_tagesmittel_290Stationen.rds")
str(clim_df)
#parameter_anni
```

```{r}
clim_filtered <- clim_df %>%
select(
...
Uhr Folgetag definiert." (Quellen: https://opendata.dwd.de/climate_environment/CDC/observations_germany/climate/daily/kl/recent/BESCHREIBUNG_obsgermany_climate_daily_kl_recent_de.pdf)
