...
```{r}
db <- paste0("../../GeoSenSys2020/Data_2020/HGU_GeoSenSys_V3_5.db") #DB in other R-Project
...
tensio <- tensio %>% 
    ...
    mutate(wasser_level = case_when(variante_id == 9 ~ "Wfull_plus",
                                    variante_id %in% c(5,10,11) ~ "Wfull",
                                    variante_id %in% c(8,12) ~ "Wred",
                                    T ~ "Leerwert")) %>%
