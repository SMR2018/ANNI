...
tensio <- tensio %>%
  ...
  mutate_at("zeit_messung", ~ format.Date(., format = "%Y-%m-%d")) %>%
  # für Tages-Mittelwert
  mutate_at("zeit_messung", ~ as_date(.)) %>%
  # wieder in Datum (class) umformen
...
tensio_melted <- tidyr::pivot_longer(tensio %>%
  select(c(
    "satz_id", "wasser_level", "zeit_messung",
...
wasser_gesamt <- tensio %>%
  select(satz_id, wasser_level, zeit_messung) %>%
  left_join(wetter, by = c("zeit_messung" = "datum_wetter")) %>%
  left_join(bewaesserung_mean, by = c("satz_id", "wasser_level",
...
plot_nfk <- function(satz_nr, subtitle) {
  ...
  p <- ggplot() +
    ...
    facet_grid(kategorie ~ wasser_level,
...
```{r}
# file1 <- "../data/derived_data/nfK_2020_2021.csv"
# data.table::fwrite(x = tensio,file = file1, sep = ";", dec = ".")
```

#Bewässerung_mean-Tabelle speichern
```{r}
# file1 <- "../data/derived_data/bewaesserung_mean_2020_2021.csv"
