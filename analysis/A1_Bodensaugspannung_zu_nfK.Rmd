...
tensio_melted <- tidyr::pivot_longer(tensio %>% 
                                       select(c("satz_id","wasser_level","zeit_messung", 
...
bewaesserung_mean <- bewaesserung %>% 
  rename(wasser_level = variante_H2O) %>%
  group_by(satz_id, datum_bewaesserung, wasser_level) %>%
  summarise(bewaesserung_mm =  mean(wassermengen_mm)) %>% 
  ungroup


#Bewaesserung und Niederschlag in eine Tabelle zusammenfuehren
wasser_gesamt <- tensio %>% select(satz_id,wasser_level, zeit_messung) %>%
  left_join(wetter, by=c("zeit_messung" = "datum_wetter")) %>% 
  left_join(bewaesserung_mean, by = c("satz_id","wasser_level", "zeit_messung"="datum_bewaesserung")) %>%
  #Faktorstufen sortieren (für Grafik)
  tidyr::pivot_longer(cols = c("bewaesserung_mm", "niederschlag_mm"), 
...
plot_nfk <- function(satz_nr, subtitle){
  ...
  p <- ggplot() +
    geom_contour_filled(data = tensio_melted %>% filter(satz_id == satz_nr),
    ...
    geom_col(data = wasser_gesamt %>% filter(satz_id == satz_nr), 
    ...
    facet_grid(kategorie~wasser_level, scales = "free_y", switch = "y",space = "free_y",
               labeller = labeller(.rows = as_labeller(c(nFK = "Bodentiefe (cm)", 
                                   ...
                                   .cols = label_value), 
               drop = FALSE) + #damit auch Wfull_plus in Satz1 dargestellt wird (ohne Daten)
...
p2 <- plot_nfk(satz_nr = 2, subtitle = "2020, Satz 2, Feld 4a (*Paper VI über diese Daten)")
