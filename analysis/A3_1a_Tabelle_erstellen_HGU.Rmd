...
```{r}
input_tabelle <- fread("../data/derived_data/A1a_tabelle_20240327.csv")
...
soil_params <- soil_params[ort == "HGU" & bodentiefe_cm == "0-30",]
soil_params

input_tabelle <- input_tabelle %>% 
    mutate(Ton_prozent = soil_params$Ton_prozent_mean, ##20 --> 9.7
           Schluff_prozent = soil_params$Schluff_prozent_mean, #30 --> 38
           Sand_prozent = soil_params$Sand_prozent_mean, ##50 --> 52.3
           C_org_prozent = 1.2)
```
...
## Fehlmessungen filtern (vgl A1a PDF):
```{r}
#2022_S2 Wfull_plus komplett raus
input_tabelle_joined_from0_corrected <- 
    input_tabelle_joined_from0_corrected %>% 
    filter(satz_id != 7 | variante_H2O != "Wfull_plus") %>% 
    
    #2022_S2 Wfull und Wred, jeweils Wdh d
    filter(satz_id != 7 | !variante_H2O %in% c("Wfull", "Wred") | wiederholung != "d") %>%
    
    #2022_S3 Wfull_plus wdh c
    filter(satz_id != 8 | !variante_H2O %in% c("Wfull_plus","Wfull") | wiederholung != "c")  %>%
    filter(satz_id != 8 | !variante_H2O %in% c("Wred") | wiederholung != "a")  %>%
    
    #2023, Satz2 Wfull_plus komplett
    filter(satz_id != 10 | variante_H2O != "Wfull_plus") %>% 
    #2023, Satz2 Wfull+Wred jeweil wdh d
    filter(satz_id != 10 | !variante_H2O %in% c("Wred","Wfull") | wiederholung != "d")  
...
## Spalten wählen und NA entfernen: \\\ nicht mehr notwendig. Bereits oben erfolgt.
```{r}
# input_tabelle_joined_from0_corrected <- input_tabelle_joined_from0_corrected %>%
#     select(-ends_with("Vortag"),  #ohne Vortag, da Zeitreihe nachträglich
#            -starts_with(c("bodensaugspannung_","pf_"))) ### wird in LSTM-Script selected

### bereits oben NAs entfernt
...
```{r}
##### ANDERS gelöst. Siehe nächster Chunk

# i30plus <- input_tabelle_final %>%
#     group_by(satz_id, variante_H2O, wiederholung) %>%
#     mutate(id = cur_group_id(), .after = "variante_H2O") %>%
#     group_by(id) %>%
#     #select(-satz_id, -variante_H2O, -wiederholung) %>%
#     mutate(nn = n()) %>% filter(nn >= 30)
# 
...
#                           "input_tabelle_2020_2023_20240403_20cm_complete.csv"),
...
#             file = paste0("../data/derived_data/",
#                           "input_tabelle_2020_2023_20240403_lstm_geeignet_20cm_complete.csv"),
#             sep = ";",dec = ".", row.names = FALSE)
```
