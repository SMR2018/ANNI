...
daten_plot_nfk <- function(satz_nr=satz_nr, wdh = FALSE, variante = NULL) {
  ...
  if(isFALSE(wdh)) { #== Mittelwert aller Wiederholungen einer Variante
    ...
    wasser_gesamt2 <- wasser_gesamt2 %>% 
      ...
      ungroup() %>%
      stacked_bars_plot(gruppen = c("kategorie", "satz_id", "variante_H2O",
                                    "zeit_messung", "tage_seit_aussaat"));    
...
plot_nfk <- function(satz_nr, subtitle, wdh = FALSE, variante = NULL) {
  ...
  list1 <- daten_plot_nfk(satz_nr = satz_nr, wdh = wdh, variante = variante)
  ...
  geom_tile(
    ...
    #guides(guide_colourbar(barwidth = 2, barheight = 10)) +
...
```{r}
#Beispiel:
plot_nfk(satz_nr = 5, subtitle = "2021, Satz 3, Feld 6", wdh = FALSE)
p2 <- plot_nfk(satz_nr = 3, subtitle = "2021, Satz 1, Feld 6, Variante 'Wred'", wdh = TRUE, variante = "Wred")
```
...
Interpolation:
```{r}
a1 <- tensio_melted %>% ungroup %>% 
  filter(satz_id == 4, variante_H2O == "Wred", wiederholung =="b") 

a <- a1 %>% 
  mutate(Bodentiefe = Bodentiefe-10)
b <- a %>% filter(Bodentiefe == 10)
c <- a %>% filter(Bodentiefe == 50)


B0cm <- b %>% mutate(Bodentiefe = 0)
B60cm <- c %>% mutate(Bodentiefe = 60)
...
a_interpoliert <- a %>% arrange(Bodentiefe) %>% 
  #mutate_at("nFK_prozent", ~ifelse(is.na(.), 9999,. ))%>%
  ...
  # mutate_at("Bodentiefe_ganz", ~ifelse(.%% 20 == 10, .+10, .))
  ungroup# %>%
