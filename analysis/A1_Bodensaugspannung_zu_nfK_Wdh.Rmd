...
tensio_melted <- tidyr::pivot_longer(tensio %>%
                                       select(c(
                                         "satz_id", "variante_H2O","wiederholung", "zeit_messung",
...
```{r}
#Wenn nach Wiederholung unterschieden, dann kein Mittelwert. Wenn nach Variante, dann Mittelwert
bewaesserung_mean <- bewaesserung %>% rename(bewaesserung_mm = wassermengen_mm)
#     bewaesserung %>%
#     group_by(satz_id, datum_bewaesserung, variante_H2O, wiederholung) %>%
...
wasser_gesamt <- tensio %>%
  ...
  left_join(bewaesserung_mean, by = c("satz_id", "variante_H2O","wiederholung",
...
plot_nfk <- function(satz_nr, subtitle, wdh = FALSE, variante = NULL) {
  #plot_nfk(satz_nr = 5, subtitle = "2021, Satz 3, Feld 6", wdh = TRUE, variante = "Wfull_plus")
  
  
  #Pro Variante oder einzeln fuer Wiederholung:
  if(isFALSE(wdh)) { #== alle Wiederholungen einer Variante
    
    tensio_melted <- tensio_melted %>% 
      filter(satz_id == satz_nr) %>%
      group_by(kategorie, satz_id, variante_H2O, Bodentiefe, zeit_messung) %>%
      summarise(nFK_prozent = mean(nFK_prozent, na.rm = TRUE)) %>%
      ungroup();
    
    wasser_gesamt <- wasser_gesamt %>% 
      filter(satz_id == satz_nr) %>%
      group_by(kategorie, variable, satz_id, variante_H2O, zeit_messung) %>%
      summarise(value = mean(value, na.rm = TRUE)) %>%
      ungroup();
    
  } else {
    # tensio_melted <- tensio_melted %>% filter(satz_id == satz_nr & wiederholung == wdh);
    # wasser_gesamt <- wasser_gesamt %>% filter(satz_id == satz_nr & wiederholung == wdh)
    tensio_melted <- tensio_melted %>% filter(satz_id == satz_nr & variante_H2O == variante);
    wasser_gesamt <- wasser_gesamt %>% filter(satz_id == satz_nr & variante_H2O == variante)
    
  }
  
  
  #Farbspektrum definieren:
...
### Plots pro Variante:
```{r}
#2020 Satz2: nur an zwei Stellen Tensiometer
plot_nfk(satz_nr = 2, subtitle = "2020, Satz 2, Feld 4a, Variante 'Wfull'", 
         wdh = TRUE, variante = "Wfull")
plot_nfk(satz_nr = 2, subtitle = "2020, Satz 2, Feld 4a,  Variante 'Wred'", 
         wdh = TRUE, variante = "Wred")
```

```{r}
#2021 Satz1
plot_nfk(satz_nr = 3, subtitle = "2021, Satz 1, Feld 6, Variante 'Wfull_plus'", 
         wdh = TRUE, variante = "Wfull_plus")
plot_nfk(satz_nr = 3, subtitle = "2021, Satz 1, Feld 6, Variante 'Wfull'", 
         wdh = TRUE, variante = "Wfull")
...
```{r}
#2021 Satz2
plot_nfk(satz_nr = 4, subtitle = "2021, Satz 2, Feld 4b, Variante 'Wfull_plus'", 
         wdh = TRUE, variante = "Wfull_plus")
plot_nfk(satz_nr = 4, subtitle = "2021, Satz 2, Feld 4b, Variante 'Wfull'", 
         wdh = TRUE, variante = "Wfull")
plot_nfk(satz_nr = 4, subtitle = "2021, Satz 2, Feld 4b, Variante 'Wred'", 
         wdh = TRUE, variante = "Wred")
```

```{r}
#2021 Satz3
p1 <- plot_nfk(satz_nr = 5, subtitle = "2021, Satz 3, Feld 6, Variante 'Wfull_plus'", 
         wdh = TRUE, variante = "Wfull_plus")
plot_nfk(satz_nr = 5, subtitle = "2021, Satz 3, Feld 6, Variante 'Wfull'", 
         wdh = TRUE, variante = "Wfull")
plot_nfk(satz_nr = 5, subtitle = "2021, Satz 3, Feld 6, Variante 'Wred'", 
         wdh = TRUE, variante = "Wred")
...
satz_nr = 5; variante = "Wfull_plus"
tensio_sd <- tensio_melted %>% 
  filter(satz_id == satz_nr & variante_H2O == variante) %>%
  pivot_wider(id_cols = c("kategorie", 
                          #"satz_id", "variante_H2O",
                          "zeit_messung", "Bodentiefe"), 
              names_from = "wiederholung", values_from = "nFK_prozent") %>%
  rowwise() %>%
  mutate(nfK_sd = sd(c(a,b,c,d))) %>% 
  ungroup

wasser_gesamt_sd <- wasser_gesamt %>% 
  filter(satz_id == satz_nr & variante_H2O == variante) %>%
  pivot_wider(id_cols = c("kategorie","variable", "zeit_messung"), 
              names_from = "wiederholung", values_from = "value") %>%
  rowwise() %>%
  mutate(wasser_sd = sd(c(a,b,c,d))) %>% 
  ungroup 
```
###Grafik
```{r}
wasser_gesamt_sd2 <- wasser_gesamt_sd %>%
  # fiktive sd fuer plot-Höhe:
  bind_rows(data.frame(kategorie = "wasserinput", variable = "bewaesserung_mm",
                       zeit_messung = as_date("1900-01-01"), wasser_sd = 20)) %>%
  mutate_at("kategorie", ~factor(., levels = c("nFK", "wasserinput")))

my_breaks <- c(seq(from = 0, to = 30, by = 5), Inf)
my_colors <- grey.colors(7, start = 0.1, end = 0.8, rev = TRUE)
my_labels <- levels(cut(tensio_sd$nfK_sd, breaks = my_breaks))
...
p <- ggplot() +
  geom_contour_filled(
    data = tensio_sd,
    aes(x = zeit_messung, y = -(Bodentiefe), z = nfK_sd),
    breaks = my_breaks
  ) +
  scale_colour_manual("Stabw Bodenfeuchte (% nFK)",
                      values = my_colors, labels = my_labels,
                      aesthetics = "fill", drop = FALSE
  ) +
  # "drop = FALSE" ist notwendig um auch Werte anzuzeigen (und die Farben zu scalen),
  ...
  # Regen und Bewässerung anfügen
  geom_col(
    data = wasser_gesamt_sd2,
    aes(zeit_messung, wasser_sd, fill = variable), show.legend = FALSE
  ) +
  scale_fill_manual(#"Stabw Wassereintrag (mm)",
                    values = c(
                      "niederschlag_mm" = "gray70",
                      "bewaesserung_mm" = "gray40"
                    ),
  ...
  scale_x_date(limits = range(tensio_sd$zeit_messung)) + 
  #damit skalierungs-Wert fuer wasserinput nicht abgezeigt wird.
  facet_grid(kategorie ~ .,
             scales = "free_y", switch = "y", space = "free_y",
             labeller = labeller(
               .rows = as_labeller(
                 c(
             ...
             shrink = FALSE #damit Höhe des Skalierungs-Wertes für Wasserinput angezeigt wird
  ) + 
  theme_bw() +
  theme(
    panel.grid = element_blank(),
...
```{r}
 ...
 plot_list_arranged <- ggpubr::ggarrange(plotlist = list(p1, p), 
                                  ncol= 2, nrow = 1,
                                  common.legend = TRUE, 
                                  align = "hv",
                                  legend="bottom") #%>% 
   # annotate_figure(top = text_grob(title, face = "bold", size = 15))
