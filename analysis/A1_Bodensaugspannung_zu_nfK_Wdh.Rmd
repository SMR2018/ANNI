...
```{r}
#2021 Satz3
plot_nfk(satz_nr = 5, subtitle = "2021, Satz 3, Feld 6, Variante 'Wfull_plus'", 
         wdh = TRUE, variante = "Wfull_plus")
...
my_colors <- c(
    colorRampPalette(c("#fac83c","brown"))(7)#,
    #colorRampPalette(c("lightgreen", "forestgreen"))(3),
    # colorRampPalette(c("#32c8fa", "darkblue"))(4)
)
my_labels <- levels(cut(tensio_sd$nfK_sd, breaks = my_breaks))

#range1 <- range(wasser_gesamt_sd$wasser_sd, na.rm = TRUE)
from1 <- c(1,20)
to1 <- range(wasser_gesamt_sd$wasser_sd, na.rm = TRUE)
...
p <- ggplot() +
    ...
    # Regen und Bewässerung anfügen
    geom_col(
        data = wasser_gesamt_sd,
        aes(zeit_messung, 
            #wasser_sd,
            scales::rescale(wasser_sd, to = c(1,20)), 
            fill = variable)
    ) +
    scale_y_continuous(
        #sec.axis = ~(. - from1[1])/diff(from1) * diff(to1) + to1[1])+
        trans =" ~.*2" #as.formula("~(. - from1[1])/diff(from1) * diff(to1) + to1[1]")
