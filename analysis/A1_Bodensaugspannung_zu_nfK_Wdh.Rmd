...
wasser_gesamt <- stacked_bars_plot(data = wasser_gesamt, 
...
plot_nfk <- function(satz_nr, subtitle, wdh = FALSE, variante = NULL) {
  ...
  # tensio_melted2 <- tensio_melted %>% 
  #   group_by(kategorie, satz_id, variante_H2O,wiederholung,Bodentiefe) %>%
  #   mutate_at("zeit_messung", ~c(0,diff(.))) %>%
  #       mutate_at("zeit_messung", ~cumsum(.))  %>% ungroup()
  # 
  # wasser_gesamt2 <- wasser_gesamt %>% 
  #   group_by(kategorie, satz_id, variante_H2O,wiederholung,variable) %>%
  #   mutate_at("zeit_messung", ~c(0,diff(.))) %>%
  #       mutate_at("zeit_messung", ~cumsum(.)) %>% ungroup()
  ...
  if(isFALSE(wdh)) { #== Mittelwert aller Wiederholungen einer Variante
    ...
    wasser_gesamt2 <- wasser_gesamt2 %>% 
      ...
      ungroup() %>%
      stacked_bars_plot(gruppen = c("kategorie", "satz_id", "variante_H2O","zeit_messung"));    
    
    
  } else { #== Wiederholungen einer Variante vergleichen
    tensio_melted2 <- tensio_melted2 %>% filter(satz_id == satz_nr & variante_H2O == variante);
    wasser_gesamt2 <- wasser_gesamt2 %>% filter(satz_id == satz_nr & variante_H2O == variante);
  ...
  p <- ggplot() +
    ...
    geom_segment(data=wasser_gesamt2,
                 aes(x = zeit_messung, y =value_min,
                     xend = zeit_messung, yend = value_stacked,
                     position = "dodge",
                     # geom_col(data=wasser_gesamt,
                     #          aes(x = zeit_messung, y= value,
                     color = variable), fill=NA, size=1.5) +
    ...
    # scale_x_date(date_labels =  wasser_gesamt %>% 
    #                slice(5:(n()-5)) %>% 
    #                pull(zeit_messung) %>% 
    #                pretty(n=3) %>% 
    #                format.Date("%d.%m")
  ...
  #              #damit sich die Zeitbeschriftungen nicht ueberschneiden;
  #           limits = c(min(wasser_gesamt$zeit_messung, na.rm = TRUE) +5 , 
  #                      max(wasser_gesamt$zeit_messung, na.rm = TRUE) -5))
  ...
  p <-    if(isTRUE(wdh)){ 
    p+  ggnewscale::new_scale("fill")+ # zweite fill-scale für SD-Plot
      ...
      geom_segment(data=wasser_gesamt_sd2,
                   aes(x = zeit_messung, y =0,
                       xend = zeit_messung, yend = wasser_sd,
                       color = variable),fill=NA, size=1)
  } else {p}
...
p2 <- plot_nfk(satz_nr = 2, subtitle = "2020, Satz 2, Feld 4a (*Paper)", wdh = FALSE)
p3 <- plot_nfk(satz_nr = 3, subtitle = "2021, Satz 1, Feld 6", wdh = FALSE)
p4 <- plot_nfk(satz_nr = 4, subtitle = "2021, Satz 2, Feld 4b", wdh = FALSE)
p5 <- plot_nfk(satz_nr = 5, subtitle = "2021, Satz 3, Feld 6", wdh = FALSE)
