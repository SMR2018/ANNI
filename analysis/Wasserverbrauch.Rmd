...
```{r}
input_tabelle <- fread("../data/derived_data/input_tabelle_2020_2021_20220504.csv") %>%
    mutate_at("zeit_messung", ~as_date(.))# %>%
#    mutate_at(c("T0020_nFK","T2040_nFK","T4060_nFK"), ~ifelse(.<52,NA,.)) 
#kleinere Werte nicht im vorgegeben Bereich des Tensiometers // doch nicht. löschen.
...
wasserverbrauch <- input_tabelle %>%  
    ...
    #Wasserinput und freien Speicher berechnen
    mutate_at(c("freier_Speicher_T0020_mm", 
                "freier_Speicher_T2040_mm", 
                "freier_Speicher_T4060_mm"), ~ifelse(.<0,0,.)) %>% 
    rowwise() %>%
    
    mutate(T0060_mm_ohne_Auswaschung = T0060_mm - 
               T0020_Auswaschung_mm - 
               T2040_Auswaschung_mm - 
               T4060_Auswaschung_mm ) %>%
    
    #Auswaschung aus oberen beiden Schichten als Input anrechnen.
    mutate(wasserinput_mm = sum(niederschlag_mm, 
                                bewaesserung_mm,
                                T0020_Auswaschung_mm,                                         ###weg? 
                                T2040_Auswaschung_mm,                                         ###weg?
                                na.rm = TRUE) ,               
    ...
    mutate_at(c("freier_Speicher_T0060_mm","T0060_mm_ohne_Auswaschung"), 
    ...
    mutate(T0060_mm_ohne_input = T0060_mm_ohne_Auswaschung - wasserinput_vortag_cumsum,
    ...
    mutate_at("kumulierter_Wasserverbrauch_smooth", 
              ~ifelse(is.na(.), kumulierter_Wasserverbrauch,.)) %>%
    mutate_at("kumulierter_Wasserverbrauch_smooth", 
...
plot_wassergehalt <- function(satz_nr, variante, wdh, x){
    ...
    data1 <- wasserverbrauch %>% filter(satz_id ==satz_nr, 
                                        ...
                                        wiederholung == wdh)
    mm_max <- max(data1$T0060_mm_ohne_Auswaschung, na.rm = TRUE) +5
    breite <- (range(pull(data1,x), na.rm = TRUE) %>% diff)*0.008
    
    p <- ggplot(data1, aes(!!sym(x),T0060_mm_ohne_Auswaschung)) +
        #Wasser-Input:
        ...
        theme_bw() + 
        scale_fill_manual("Wassereintrag", 
                          values = c("P+Bewässerung" = "lightblue")) + 
...
plot_wasser_cumsum <- function(x, satz_nr, variante, wdh, log=FALSE){ #fit
    ...
    data1 <- wasserverbrauch %>% filter(satz_id ==satz_nr, 
                                        variante_H2O == variante,
                                        wiederholung == wdh) %>%
        mutate(x = if(log == TRUE){log(!!sym(x))}else{!!sym(x)})
    ...
    ggplot(data = data1, 
        ...
        ylab("Kumulierter Wasserverbrauch (mm)") + xlab(ifelse(log == TRUE, paste("log_", x), x)) +
        theme_bw() + 
