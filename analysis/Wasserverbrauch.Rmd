...
```{r}
input_tabelle <- read.csv("../data/derived_data/input_tabelle_2021_Satz1_b.csv", 
                          sep = ";", dec = ".") %>%
...
```{r}
 wasserverbrauch <- input_tabelle %>%         
     rowwise() %>%
    mutate(wasserinput_mm = sum(P, bewaesserung_mm, na.rm = TRUE) ,#- FAO56, 
...
```{r}
ggplot(wasserverbrauch, aes(cumsum(FAO56),kumulierter_Wasserverbrauch)) + 
    geom_smooth() + 
    geom_point() + 
