...
train_y <- array(data = train_y, dim = c(NROW(train_y), 1))
test_x <- array(data = test_x, dim = c(NROW(test_x), 1, seq_size))
test_y <- array(data = test_y, dim = c(NROW(test_y), 1))

# train_x <- array(data = train_x, dim = c(NROW(train_x), seq_size, 1))
# train_y <- array(data = train_y, dim = c(NROW(train_y), 1))
# test_x <- array(data = test_x, dim = c(NROW(test_x), seq_size,1))
# test_y <- array(data = test_y, dim = c(NROW(test_y), 1))
```
...
# lstm_model %>% 
#     layer_lstm(units = 64, 
#                batch_input_shape = c(batch_size, seq_size, 1),
#                return_sequences = FALSE
#     ) %>%
#     keras::layer_repeat_vector(n = seq_size) %>%
#     layer_lstm(units = 64,
#                return_sequences = TRUE,
#                stateful = TRUE
#     ) %>%
...
pred <- lstm_model %>% predict(test_x, batch_size = batch_size)
#pred <- lstm_model %>% predict(train_x, batch_size = batch_size) 

#rescale to normal nFK-values:
pred_nFK <- pred[,,1] * s + m
test_y_nFK <- test_y * s + m
#test_y_nFK <- train_y * s + m 

#Gütemaße:
mean((test_y_nFK-pred_nFK)^2) 
paste("RMSE =", round(mean(sqrt((test_y_nFK-pred_nFK)^2)), 4))
#plot(test_y_nFK, pred_nFK)
```

#Ergebnis plotten:
```{r}
x1 <- test_y_nFK %>% as.data.frame() %>%
    tidyr::pivot_longer(cols = everything(), values_to = "measured", names_to = "depth")

y1 <- pred_nFK %>% as.data.frame() %>%
...
```{r}
df_pred_id <- data.frame(
    measured = test_y_nFK, 
    predicted = pred_nFK) %>%
    mutate(id_all = rep(1:ceiling(n()/9), each = 9)[1:n()]) %>%
    group_by(id_all) %>%
    mutate(Tage = 1:n()) %>% 
    ungroup() %>%
    mutate(x= 1:n())

ggplot(df_pred_id, aes(x=x)) +
    geom_line(aes(y=measured, col = "gemessen")) +
    geom_line(aes(y=predicted, col = "modelliert")) +
    theme_bw()+
    theme(panel.grid = element_blank())
```
