...
```{r, eval=TRUE}
rm(model, new_model)
filepath_tf <- "../data/derived_data/Models_paper/X3_reduced_Model3_1a_MPL_pretrained_AMBAV_meteo/"
...
p <- plot_measured_predicted_lm(linear_model = lm1, data = result) +
...
```{r}
p$layers[7:9] <- NULL
p2 <- p + ylab("Predicted AWC (%)") + xlab("Measured AWC (%)")
p2
```

```{r}
#ggsave(plot = p2, filename = "~/Downloads/guete.png")
```
...
```{r, eval=TRUE}
filepath_tf <- "../data/derived_data/Models_paper/X4_reduced_Model3_1b_MLP_tuned_experimental_data"
