...
rmarkdown::render(input = "Wetterdaten_Praxisschlag_beziehen.Rmd",
                  params = list(schlag = params$schlag, lat = params$lat, lon = params$lon, datum_aussaat = params$datum_aussaat))
```
...
df_zusammen<- df_zusammen %>% 
    mutate(
        nFK_0010_Vortag = c(params$start_nFK[1], rep(NA, times = n()-1)),
        nFK_1020_Vortag = c(params$start_nFK[2], rep(NA, times = n()-1)),
        nFK_2030_Vortag = c(params$start_nFK[3], rep(NA, times = n()-1)),
        nFK_3040_Vortag = c(params$start_nFK[4], rep(NA, times = n()-1)),
        nFK_4050_Vortag = c(params$start_nFK[5], rep(NA, times = n()-1)),
        nFK_5060_Vortag = c(params$start_nFK[6], rep(NA, times = n()-1)),
        wasser_input_Vortag = c(0, wasser_input_mm[-n()])) #%>%
    # mutate(across(starts_with("nFK"), ~ifelse(tage_seit_aussaat == 1, 100, NA)))
        
# df_zusammen[1, c("nFK_0010_Vortag","nFK_1020_Vortag","nFK_2030_Vortag",
                 # "nFK_3040_Vortag","nFK_4050_Vortag","nFK_5060_Vortag")] <- params$start_nFK
```
...
#Leer-Werte füllen, falls nicht in Messungen enthalten
```{r}
data_praxis <- data_praxis %>% mutate(across(!ends_with("_Vortag"), 
                                             ~ifelse(is.na(.), round(mean(., na.rm = TRUE),2), .)))
```
